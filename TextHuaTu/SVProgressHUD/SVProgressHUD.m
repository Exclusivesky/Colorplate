...
- (void)setStatus:(NSString *)string {
    ...
    if(string) {
        CGSize stringSize = [self sizeWithContentString:string font:self.stringLabel.font size:CGSizeMake(200, 300)];
...
#pragma makr - 自定义
+ (void)showTextOnly:(NSString*)status{
    [[SVProgressHUD sharedView] showOnlyText:status];
    [[SVProgressHUD sharedView] dismissWithStatus:status afterDelay:1];
}
- (void)showOnlyText:(NSString *)string{
    dispatch_async(dispatch_get_main_queue(), ^{
        if(!self.superview)
            [self.overlayWindow addSubview:self];
        self.fadeOutTimer = nil;
...
- (void)dismissWithStatus:(NSString *)string afterDelay:(NSTimeInterval)inttegerValue
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if(self.alpha != 1)
            return;
        ...
        if(string) {
            ...
            if(hudHeight > 100) {
                labelRect = CGRectMake(12, 30, hudWidth, stringHeight);
                hudWidth+=24;
            } else {
                hudWidth+=24;
                labelRect = CGRectMake(0, 30, hudWidth, stringHeight);
            }
        }
        self.hudView.bounds = CGRectMake(0, 0, hudWidth, hudHeight-20);
        self.stringLabel.hidden = NO;
        self.stringLabel.text = string;
        self.stringLabel.font = [UIFont boldSystemFontOfSize:14];
        self.stringLabel.center = self.hudView.center;
        self.stringLabel.frame = labelRect;
        self.fadeOutTimer = [NSTimer scheduledTimerWithTimeInterval:inttegerValue target:self selector:@selector(dismiss) userInfo:nil repeats:NO];
    });
}
